<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Stepper -->
      <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6 mb-6">
        <ol class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <li
            :class="[
              'flex items-center p-3 rounded-lg border',
              wizardStep === 1 ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200',
            ]"
          >
            <span
              class="mr-3 inline-flex items-center justify-center h-8 w-8 rounded-full"
              :class="wizardStep >= 1 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'"
              >1</span
            >
            <div>
              <p class="text-sm font-semibold text-slate-800">Pilih status pengajuan</p>
              <p class="text-xs text-slate-500">Tentukan tipe pengajuan</p>
            </div>
          </li>
          <li
            :class="[
              'flex items-center p-3 rounded-lg border',
              wizardStep === 2 ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200',
            ]"
          >
            <span
              class="mr-3 inline-flex items-center justify-center h-8 w-8 rounded-full"
              :class="wizardStep >= 2 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'"
              >2</span
            >
            <div>
              <p class="text-sm font-semibold text-slate-800">Lengkapi data diri</p>
              <p class="text-xs text-slate-500">Pastikan profil lengkap</p>
            </div>
          </li>
          <li
            :class="[
              'flex items-center p-3 rounded-lg border',
              wizardStep === 3 ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200',
            ]"
          >
            <span
              class="mr-3 inline-flex items-center justify-center h-8 w-8 rounded-full"
              :class="wizardStep >= 3 ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'"
              >3</span
            >
            <div>
              <p class="text-sm font-semibold text-slate-800">Lengkapi pengantar RT</p>
              <p class="text-xs text-slate-500">Upload surat pengantar RT/RW</p>
            </div>
          </li>
        </ol>
      </div>
      ...
      <div
        v-if="wizardStep === 2"
        class="bg-amber-50 border border-amber-200 rounded-2xl p-6 sm:p-8 mb-6"
      >
        <h3 class="text-lg sm:text-xl font-bold text-amber-800 mb-2">Lengkapi Data Diri</h3>
        <p class="text-amber-700 text-sm mb-4">
          Sebelum melanjutkan pengajuan, mohon lengkapi data diri Anda terlebih dahulu.
        </p>
        <button
          @click="router.push('/user-profile')"
          class="inline-flex items-center px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-medium rounded-xl transition-all duration-200"
        >
          Lengkapi Profil
        </button>
      </div>

      <!-- Step 3: Upload Surat Pengantar RT/RW -->
      <div
        v-if="wizardStep === 3"
        class="bg-blue-50 border border-blue-200 rounded-2xl p-6 sm:p-8 mb-6"
      >
        <div class="flex items-center mb-4">
          ...
          <h3 class="text-xl sm:text-2xl font-bold text-slate-800">Upload Surat Pengantar RT/RW</h3>
        </div>
        <p class="text-slate-700 text-sm mb-4">
          Unggah dokumen "Surat Pengantar RT/RW (asli)" untuk melanjutkan.
        </p>
        <form @submit.prevent="handleUploadRtPengantar" class="space-y-4">
          <div>
            <label
              for="rt-document"
              class="block text-slate-700 font-medium mb-2 text-sm sm:text-base"
              >Pilih File Dokumen <span class="text-red-500">*</span></label
            >
            <input
              id="rt-document"
              type="file"
              @change="onRtFileChange"
            ...
            <p class="text-xs text-slate-500 mt-1 sm:mt-2">
              Format didukung: PDF, JPG, JPEG, PNG, DOC, DOCX. Maksimal 10MB.
            </p>
          </div>
          <div
            v-if="uploadError"
          ...
          <button
            type="submit"
            :disabled="uploading || !rtFile"
            class="inline-flex items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <svg
              v-if="uploading"
            ...
            {{ uploading ? 'Mengupload...' : 'Upload Pengantar RT/RW' }}
          </button>
        </form>
      </div>

      <div
        v-if="wizardStep >= 4 && allDocumentsUploaded && allDocumentsApproved"
      ...
      <div
        v-else-if="wizardStep >= 4 && selectedSubmissionType"
      ...
      <div v-if="wizardStep >= 4 && selectedSubmissionType" class="space-y-8">
        <div
        ...
        <div
          v-if="readyToSubmit"
          class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl shadow-lg p-6 sm:p-8 mb-8 hover:shadow-xl transition-shadow duration-300"
        >
          <div class="flex items-center mb-6">
            <div
              class="w-10 h-10 sm:w-12 sm:h-12 bg-green-100 rounded-xl flex items-center justify-center mr-3 sm:mr-4"
            ...
            <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Siap untuk Submit</h2>
          </div>

          <div class="mb-6">
            <p class="text-green-800 font-semibold mb-2">
              Semua dokumen telah siap! Anda memiliki {{ pendingDocuments.length }} dokumen yang
              akan dikirim.
            </p>
            <p class="text-green-700 text-sm">
              Klik tombol "Submit Semua Dokumen" untuk mengirim semua dokumen ke server.
            </p>
          </div>

          <!-- Progress bar untuk submission (hanya muncul saat submitting) -->
          <div v-if="isSubmitting" class="mb-6">
            <div class="flex items-center justify-between mb-2">
              <p class="text-sm font-medium text-green-800">Mengirim dokumen...</p>
              <span class="text-sm font-medium text-green-800">{{ submissionProgress }}%</span>
            </div>
          ...
          >
            ...
            {{
              isSubmitting
                ? `Mengirim... (${submissionProgress}%)`
                : `Submit Semua Dokumen (${pendingDocuments.length})`
            }}
          </button>
        </div>
          ...
          <div class="grid gap-4 sm:gap-6">
            ...
            >
              <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                ...
                >
                  <a
                    :href="`http://localhost:3001/uploads/${doc.file_name}`"
                    target="_blank"
...
const API_SURAT_URL = 'http://localhost:3001/api/documents/surat'
const API_URL = 'http://localhost:3001/api/documents'
const API_PENGAJUAN_URL = 'http://localhost:3001/api/documents/pengajuan'
const API_PROFILE = 'http://localhost:3001/api/profile'
const API_KELENGKAPAN_URL = 'http://localhost:3001/api/kelengkapan'
...
const rtFile = ref(null)

const rtDocTypeValue = 'surat_pengantar_rt_rw'
const rtRequired = computed(() => {
  return documentRequirements.value.some((doc) => doc.value === rtDocTypeValue)
})
const hasRtDoc = computed(() => {
  return hasUploadedDocument(rtDocTypeValue) || hasPendingDocument(rtDocTypeValue)
})
const wizardStep = computed(() => {
  if (!selectedSubmissionType.value) return 1
  if (!profileComplete.value) return 2
  if (rtRequired.value && !hasRtDoc.value) return 3
  return 4
})
...
// localStorage key for pending documents
const getLocalStorageKey = () => {
  return `pending_documents_${authStore.user.id}_${selectedSubmissionType.value}`
}

// Load pending documents from localStorage
const loadPendingDocuments = () => {
  if (!selectedSubmissionType.value || !authStore.user?.id) return

  const key = getLocalStorageKey()
  const stored = localStorage.getItem(key)
  if (stored) {
    try {
      pendingDocuments.value = JSON.parse(stored)
    } catch (error) {
...
// Save pending documents to localStorage
const savePendingDocuments = () => {
  if (!selectedSubmissionType.value || !authStore.user?.id) return

  const key = getLocalStorageKey()
  localStorage.setItem(key, JSON.stringify(pendingDocuments.value))
}

// Clear pending documents from localStorage
...
// Check if document exists in pending documents
const hasPendingDocument = (typeValue) => {
  return pendingDocuments.value.some((doc) => doc.document_type === typeValue)
}

// Get pending document by type
const getPendingDocument = (typeValue) => {
  return pendingDocuments.value.find((doc) => doc.document_type === typeValue)
}

// Combined documents (uploaded + pending)
const combinedDocuments = computed(() => {
  const uploaded = filteredUploadedDocuments.value
  const pending = pendingDocuments.value.map((doc) => ({
    ...doc,
...
// Check if all documents are ready (uploaded or pending)
const allDocumentsReady = computed(() => {
  if (!selectedSubmissionType.value || documentRequirements.value.length === 0) return false

  return documentRequirements.value.every((req) => {
    const hasUploaded = hasUploadedDocument(req.value)
    const hasPending = hasPendingDocument(req.value)
    return hasUploaded || hasPending
  })
})

// Check if ready to submit (all documents ready)
const readyToSubmit = computed(() => {
  return allDocumentsReady.value && pendingDocuments.value.length > 0
})
...
watch(selectedSubmissionType, (newValue) => {
  ...
  // Load pending documents when submission type changes
  loadPendingDocuments()

  // Hanya tampilkan pengingat RT jika profil lengkap dan RT memang dibutuhkan
  if (newValue && profileComplete.value && rtRequired.value && !hasRtDoc.value) {
    showRTRWReminderModal.value = true
  } else {
    showRTRWReminderModal.value = false
  }
})
...
// Modified to use combinedDocuments
const paginatedFilteredUploadedDocuments = computed(() => {
  ...
  return combinedDocuments.value.slice(start, end)
})

const totalPagesUploadedDocs = computed(() => {
  return Math.ceil(combinedDocuments.value.length / itemsPerPageUploadedDocs)
})
...
const allDocumentsUploaded = computed(() => {
  return (
    documentRequirements.value.every((req) => {
      const hasUploaded = hasUploadedDocument(req.value)
      const hasPending = hasPendingDocument(req.value)
      return hasUploaded || hasPending
...
// Modified upload handler to save to localStorage first
const handleUpload = async () => {
  ...
  // Check if document already exists (uploaded or pending)
  if (
    hasUploadedDocument(selectedDocumentType.value) ||
    hasPendingDocument(selectedDocumentType.value)
  ) {
    uploadError.value = `Dokumen "${getDocumentTypeLabel(selectedDocumentType.value)}" sudah pernah diupload atau ditambahkan untuk tipe pengajuan ini.`
    return
  }
  ...
  try {
    ...
    const documentData = {
      ...
      id_pengajuan: selectedPengajuanId.value,
    ...
    pendingDocuments.value.push(documentData)
    savePendingDocuments()

    uploadSuccess.value =
      'Dokumen berhasil ditambahkan! Akan dikirim setelah semua dokumen lengkap.'
    ...
    uploadError.value = 'Gagal menambahkan dokumen. Silakan coba lagi.'
...
// Special handler for RT Pengantar upload in step 3
const onRtFileChange = (event) => {
  selectedDocumentType.value = rtDocTypeValue
  onFileChange(event)
  rtFile.value = event.target.files[0] || null
}

const handleUploadRtPengantar = async () => {
  if (!rtFile.value) {
    uploadError.value = 'Silakan pilih file pengantar RT/RW.'
    return
  }
  selectedDocumentType.value = rtDocTypeValue
  await handleUpload()
...
// Submit all pending documents to API
const submitAllDocuments = async () => {
  if (pendingDocuments.value.length === 0) {
    Swal.fire({
      icon: 'warning',
      title: 'Tidak Ada Dokumen',
      text: 'Tidak ada dokumen yang perlu dikirim.',
      confirmButtonText: 'OK',
    })
    return
  }
  ...
  const result = await Swal.fire({
    ...
    text: `Apakah Anda yakin ingin mengirim ${pendingDocuments.value.length} dokumen?`,
  ...
  try {
    const totalDocs = pendingDocuments.value.length
    let successCount = 0
    let failedCount = 0
    const failedDocs = []

    for (let i = 0; i < pendingDocuments.value.length; i++) {
      const doc = pendingDocuments.value[i]

      try {
        // Convert base64 back to file
        const file = base64ToFile(doc.file_data, doc.file_name, doc.file_type)
        ...
        formData.append('id_pengajuan', doc.id_pengajuan)
      ...
      // Update progress
      submissionProgress.value = Math.round(((i + 1) / totalDocs) * 100)
    }

    // Clear pending documents if all successful
    if (successCount > 0) {
      // Remove successfully uploaded documents
      if (failedCount === 0) {
        clearPendingDocuments()
      } else {
        // Keep only failed documents
        pendingDocuments.value = pendingDocuments.value.filter((doc) =>
          failedDocs.some((failed) => failed.fileName === doc.file_name),
        )
        savePendingDocuments()
      }
    ...
    if (failedCount === 0) {
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: `Semua ${successCount} dokumen berhasil dikirim!`,
        confirmButtonText: 'OK',
      })
      ...
      Swal.fire({
        ...
        html: `
          <p>${successCount} dokumen berhasil dikirim</p>
          <p>${failedCount} dokumen gagal dikirim:</p>
          <ul style="text-align: left; margin-top: 10px;">
            ${failedDocs.map((doc) => `<li>${doc.fileName}: ${doc.error}</li>`).join('')}
          </ul>
        `,
        confirmButtonText: 'OK',
      })
      ...
      Swal.fire({
        icon: 'error',
        title: 'Gagal!',
        text: 'Semua dokumen gagal dikirim. Silakan coba lagi.',
        confirmButtonText: 'OK',
      })
    }
  } catch (error) {
    ...
    Swal.fire({
      ...
      text: 'Gagal mengirim dokumen. Silakan coba lagi.',
      confirmButtonText: 'OK',
    })
  } finally {
...
const deletePendingDocument = (docId) => {
...
const deleteDocument = async () => {
  ...
  // Check if it's a pending document (stored locally)
  if (documentToDelete.value.isPending) {
    deletePendingDocument(documentToDelete.value.id)
    showDeleteModal.value = false
    documentToDelete.value = null
    return
  }
...
const getDocumentStatus = (typeValue) => {
  ...
  const pendingDoc = getPendingDocument(typeValue)
  if (pendingDoc) return 'pending_local'
  ...
  try {
    ...
    if (response.data.user) {
      ...
      if (emptyFields.length > 0) {
        ...
        currentStep.value = 1 // Set to step 1: complete profile
        Swal.fire({
          icon: 'warning',
          title: 'Data Tidak Lengkap',
          text: 'Mohon lengkapi data diri Anda terlebih dahulu sebelum melakukan pengajuan.',
    ...
    Swal.fire({
      icon: 'error',
      title: 'Terjadi Kesalahan',
      text: 'Gagal mengambil data profil. Silakan coba lagi.',
      confirmButtonText: 'OK',
    })
    return false
  }
...
watch(
  pendingDocuments,
  () => {
    showSubmitButton.value = readyToSubmit.value
  },
  { deep: true },
)
...
onMounted(async () => {
  ...
  if (isProfileComplete) {
    ...
    if (canSelectSubmissionType.value) {
      currentStep.value = 2 // Move to step 2: select submission type
    } else {
      // If kelengkapan is not complete, maybe there's a specific step for that?
      // For now, if profile is complete but kelengkapan isn't, stick to step 2 but disable selection
      currentStep.value = 2
      selectedSubmissionType.value = '' // Clear selection if kelengkapan is not ready
    }
  }
  // No need for fetchUserDocuments or fetchUserSurat here, they are triggered by watch(selectedPengajuanId)
})
</script>
